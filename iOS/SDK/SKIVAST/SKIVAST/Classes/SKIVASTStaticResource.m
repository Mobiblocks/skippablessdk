...
@interface SKIVASTStaticResource ()
@property (nonatomic, readwrite) NSString *creativeType;

@property (nonatomic, readwrite) NSURL *value;

@end

@implementation SKIVASTStaticResource {
    NSLocale *_locale;
}
/**
...
- (void) readAttributes:(void*) reader {
    char* creativeTypeAttrValue = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*)"creativeType");
    if(creativeTypeAttrValue) {
        self.creativeType = [NSString
        stringWithCString:creativeTypeAttrValue
        encoding:NSUTF8StringEncoding];
        xmlFree(creativeTypeAttrValue);
    }
}
...
* Description: Iterate through the XML and create the SKIVASTStaticResource object
*/
...
- (NSDictionary *)dictionary {
    ...
    if(self.creativeType != nil) {
        [dict setObject:self.creativeType forKey:@"creativeType"];
    }
