...
@interface SKIVASTUniversalAdId ()
@property (nonatomic, readwrite) NSString *idRegistry;
@property (nonatomic, readwrite) NSString *idValue;

@property (nonatomic, readwrite) NSString *value;

@end

@implementation SKIVASTUniversalAdId {
    NSLocale *_locale;
}
/**
...
- (void) readAttributes:(void*) reader {
    char* idRegistryAttrValue = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*)"idRegistry");
    if(idRegistryAttrValue) {
        self.idRegistry = [NSString
        stringWithCString:idRegistryAttrValue
        encoding:NSUTF8StringEncoding];
        xmlFree(idRegistryAttrValue);
    }
    char* idValueAttrValue = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*)"idValue");
    if(idValueAttrValue) {
        self.idValue = [NSString
        stringWithCString:idValueAttrValue
        encoding:NSUTF8StringEncoding];
        xmlFree(idValueAttrValue);
    }
}
...
* Description: Iterate through the XML and create the SKIVASTUniversalAdId object
*/
...
- (NSDictionary *)dictionary {
    ...
    if(self.idRegistry != nil) {
        [dict setObject:self.idRegistry forKey:@"idRegistry"];
    }
    
    if(self.idValue != nil) {
        [dict setObject:self.idValue forKey:@"idValue"];
    }
