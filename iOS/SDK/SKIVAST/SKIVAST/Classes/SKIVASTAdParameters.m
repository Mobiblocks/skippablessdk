...
@interface SKIVASTAdParameters ()
@property (nonatomic, readwrite) NSNumber *xmlEncoded;

@property (nonatomic, readwrite) NSString *value;

@end

@implementation SKIVASTAdParameters {
    NSLocale *_locale;
}
/**
...
- (void) readAttributes:(void*) reader {
    char* xmlEncodedAttrValue = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*)"xmlEncoded");
    if(xmlEncodedAttrValue) {
        self.xmlEncoded = [NSNumber numberWithBool:[[NSString stringWithCString:xmlEncodedAttrValue encoding:NSUTF8StringEncoding] isEqualToString:@"true"]];
        xmlFree(xmlEncodedAttrValue);
    }
}
...
* Description: Iterate through the XML and create the SKIVASTAdParameters object
*/
...
- (NSDictionary *)dictionary {
    ...
    if(self.xmlEncoded != nil) {
        [dict setObject:self.xmlEncoded forKey:@"xmlEncoded"];
    }
