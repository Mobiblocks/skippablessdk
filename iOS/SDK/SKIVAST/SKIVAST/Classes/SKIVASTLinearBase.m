...
@property (nonatomic, readwrite) SKIVASTIcons *icons;
@property (nonatomic, readwrite) SKIVASTTrackingEvents *trackingEvents;

@end

@implementation SKIVASTLinearBase {
    NSLocale *_locale;
}
/**
...
- (void) readAttributes:(void*) reader {
    char* skipoffsetAttrValue = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*)"skipoffset");
    if(skipoffsetAttrValue) {
        self.skipoffset = [NSString
        stringWithCString:skipoffsetAttrValue
        encoding:NSUTF8StringEncoding];
        xmlFree(skipoffsetAttrValue);
    }
}
...
* Description: Iterate through the XML and create the SKIVASTLinearBase object
*/
...
- (void)handleElementName:(NSString *)_currentElementName reader:(void *) reader readerOk:(int *)_readerOk currentNodeType:(int *)_currentNodeType currentXmlDept:(int *)_currentXmlDept handledInChild:(BOOL *)handledInChild {
    ...
    if([@"Icons" isEqualToString:_currentElementName]) {
        
        self.icons = [[SKIVASTIcons alloc] initWithReader:reader];
        *handledInChild = YES;
...
- (NSDictionary *)dictionary {
    ...
    if(self.skipoffset != nil) {
        [dict setObject:self.skipoffset forKey:@"skipoffset"];
    }
    
    if(self.icons != nil) {
        NSDictionary *iconsDict = [self.icons valueForKeyPath:@"dictionary"];
        [dict setObject:iconsDict forKey:@"icons"];
    }
