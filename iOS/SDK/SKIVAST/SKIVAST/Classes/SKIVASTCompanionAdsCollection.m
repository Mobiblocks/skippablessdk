...
@interface SKIVASTCompanionAdsCollection ()
@property (nonatomic, readwrite) NSString *required;

@property (nonatomic, readwrite) NSMutableArray<SKIVASTCompanionAd *> *companions;

@end

@implementation SKIVASTCompanionAdsCollection {
    NSLocale *_locale;
}
/**
...
- (void) readAttributes:(void*) reader {
    char* requiredAttrValue = (char*) xmlTextReaderGetAttribute(reader, (xmlChar*)"required");
    if(requiredAttrValue) {
        self.required = [NSString
        stringWithCString:requiredAttrValue
        encoding:NSUTF8StringEncoding];
        xmlFree(requiredAttrValue);
    }
}
...
* Description: Iterate through the XML and create the SKIVASTCompanionAdsCollection object
*/
...
- (void)handleElementName:(NSString *)_currentElementName reader:(void *) reader readerOk:(int *)_readerOk currentNodeType:(int *)_currentNodeType currentXmlDept:(int *)_currentXmlDept handledInChild:(BOOL *)handledInChild {
    ...
    if([@"Companion" isEqualToString:_currentElementName]) {
        
        [self.companions addObject:[[SKIVASTCompanionAd alloc] initWithReader:reader]];
        *handledInChild = YES;
...
- (NSDictionary *)dictionary {
    ...
    if(self.required != nil) {
        [dict setObject:self.required forKey:@"required"];
    }
    
    if(self.companions != nil) {
        NSDictionary *companionsDict = [self.companions valueForKeyPath:@"dictionary"];
        [dict setObject:companionsDict forKey:@"companions"];
    }
